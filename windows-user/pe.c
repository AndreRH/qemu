...
static BOOL import_dll( HMODULE module, const IMAGE_IMPORT_DESCRIPTOR *descr, LPCWSTR load_path, WINE_MODREF **pwm )
{
    ...
    while (import_list->u1.Ordinal)
    {
        ...
        if (IMAGE_SNAP_BY_ORDINAL(import_list->u1.Ordinal))
        {
            ...
            {
                thunk_list->u1.Function = (ULONG_PTR)dbgbreak;
                WINE_WARN("No implementation for %s.%d imported from %s, setting to %p\n",
        ...
        else  /* import by name */
        {
            ...
            {
                thunk_list->u1.Function = (ULONG_PTR)dbgbreak;
                WINE_WARN("No implementation for %s.%s imported from %s, setting to %p\n",
                                                 ...
                                                 ULONG buf_size, ULONG* req_size)
{
    ...
    for (entry = mark->Flink; entry != mark; entry = entry->Flink)
    {
        ...
        if (size <= buf_size)
        {
            sm->Reserved1 = 0; /* WINE_FIXME */
            sm->Reserved2 = 0; /* WINE_FIXME */
            sm->ImageBaseAddress = mod->BaseAddress;
            sm->ImageSize = mod->SizeOfImage;
            sm->Flags = mod->Flags;
            sm->Id = id++;
            sm->Rank = 0; /* WINE_FIXME */
            sm->Unknown = 0; /* WINE_FIXME */
            str.Length = 0;
...
const void *qemu_GetProcAddress(HMODULE module, const char *function)
{
    ...
    if ((ULONG_PTR)function >> 16)
    {
        ...
        nts = LdrGetProcedureAddress( module, &str, 0, (void**)&fp );
    }
    else
        nts = LdrGetProcedureAddress( module, NULL, LOWORD(function), (void**)&fp );
    if (nts != STATUS_SUCCESS)
    {
